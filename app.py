...
if __name__ == "__main__":
    app.run(host="0.0.0.0", port=10000, debug=True)_focuses = {
        'beauty_skincare': """
BEAUTY/SKINCARE PSYCHOLOGY FOCUS:
- Transformation aspiration: How does this tap into desires for change/improvement?
- Routine psychology: What makes routines psychologically satisfying?
...
# ==============================
# PRESERVE ALL EXISTING ANALYSIS FUNCTIONS
# ==============================
...
def run_enhanced_gpt_analysis(transcript_text, frames_summaries_text, creator_note, platform, target_duration, goal, tone, audience, knowledge_context=""):
    """Enhanced analysis that tries psychological analysis first, then falls back to original comprehensive analysis."""
    
    # FIRST: Try enhanced psychological analysis
    try:
        print("🧠 Attempting enhanced psychological analysis...")
        ...
        if enhanced_result:
            print("✅ Enhanced psychological analysis successful!")
            return enhanced_result
        else:
            print("⚠️ Enhanced analysis returned None, falling back to original...")
    except Exception as e:
        print(f"⚠️ Enhanced analysis failed: {e}, falling back to original comprehensive analysis...")
    
    # FALLBACK: Use your original comprehensive analysis (PRESERVED)
    ...
    print(f"Text sync analysis: {text_sync_analysis['text_analysis_summary']}")
...
Use this performance data to validate your analysis - explain WHY this video achieved this level of success.
    ...
    # Add text analysis context
    text_context = ""
    if text_sync_analysis['has_graphics'] or text_sync_analysis['has_captions']:
        text_context = f"\nTEXT ANALYSIS: {text_sync_analysis['text_analysis_summary']}"
        if text_sync_analysis['onscreen_graphics']:
            text_context += f"\nOn-screen graphics: {[g['text'] for g in text_sync_analysis['onscreen_graphics']]}"
        if text_sync_analysis['synchronized_captions']:
            text_context += f"\nSynchronized captions: {[c['text'] for c in text_sync_analysis['synchronized_captions']]}"
    ...
    try:
        ...
        try:
            ...
            result = {
                ...
                # Preserve enhanced fields (will be empty in fallback but structure maintained)
            ...
            print(f"Comprehensive analysis complete - Topics: {content_themes}")
            return result
            
        except json.JSONDecodeError as e:
            print(f"JSON parsing failed: {e}")
            return create_enhanced_fallback(video_description, patterns, content_themes, goal, performance_data, text_sync_analysis)
...
def create_enhanced_fallback(video_description, patterns, content_themes, goal, performance_data, text_sync_analysis):
    """Enhanced fallback with richer analysis framework and text sync analysis."""
    ...
    # Add text sync insights
    if text_sync_analysis['has_graphics'] or text_sync_analysis['has_captions']:
        analysis += f"{text_sync_analysis['text_analysis_summary']}. "
        if text_sync_analysis['has_graphics']:
            analysis += "On-screen graphics provide additional context and visual interest. "
        if text_sync_analysis['has_captions']:
            analysis += "Synchronized captions reinforce the spoken message. "
